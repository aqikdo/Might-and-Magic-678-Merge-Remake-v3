...
local function MonsterBuffsAdjust_LowFrequency()
	local nomon = 1
...
local function MonsterBuffsAdjust_HighFrequency()
	for i,mon in Map.Monsters do
		if mon.Active and mon.HP > 0 then
			...
			if charmBuff.ExpireTime >= gameTime + const.Minute * 11 then -- const.Day * 10 ?
...
local function MonsterRandomWalk()
	for i,mon in Map.Monsters do
		if mon.Active == true and mon.HP > 0 and mon.VelocityX == 0 and mon.VelocityY == 0 and mon.VelocityZ == 0 and math.random(1,50) == 1 and mon.Ally ~= 9999 and mon.SpellBuffs[const.MonsterBuff.Summoned].ExpireTime < Game.Time then
